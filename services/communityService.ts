...
// SQL Script formatado para exibição amigável na interface do usuário
export const COMMUNITY_SQL_MIGRATION = `-- 1. Tabela de Posts da Comunidade
...
DROP POLICY IF EXISTS "Qualquer pessoa pode ver posts" ON public.community_posts;
CREATE POLICY "Qualquer pessoa pode ver posts" ON public.community_posts FOR SELECT USING (true);

DROP POLICY IF EXISTS "Usuários autenticados podem postar" ON public.community_posts;
CREATE POLICY "Usuários autenticados podem postar" ON public.community_posts FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Apenas criador pode editar" ON public.community_posts;
CREATE POLICY "Apenas criador pode editar" ON public.community_posts FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Apenas criador pode deletar" ON public.community_posts;
CREATE POLICY "Apenas criador pode deletar" ON public.community_posts FOR DELETE USING (auth.uid() = user_id);

-- Políticas para REAÇÕES
DROP POLICY IF EXISTS "Qualquer pessoa pode ver reações" ON public.community_reactions;
CREATE POLICY "Qualquer pessoa pode ver reações" ON public.community_reactions FOR SELECT USING (true);

DROP POLICY IF EXISTS "Usuários autenticados podem reagir" ON public.community_reactions;
CREATE POLICY "Usuários autenticados podem reagir" ON public.community_reactions FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Apenas dono pode deletar reação" ON public.community_reactions;
CREATE POLICY "Apenas dono pode deletar reação" ON public.community_reactions FOR DELETE USING (auth.uid() = user_id);

-- Políticas para COMENTÁRIOS
DROP POLICY IF EXISTS "Qualquer pessoa pode ver comentários" ON public.community_comments;
CREATE POLICY "Qualquer pessoa pode ver comentários" ON public.community_comments FOR SELECT USING (true);

DROP POLICY IF EXISTS "Usuários autenticados podem comentar" ON public.community_comments;
CREATE POLICY "Usuários autenticados podem comentar" ON public.community_comments FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Apenas criador pode editar comentário" ON public.community_comments;
CREATE POLICY "Apenas criador pode editar comentário" ON public.community_comments FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Apenas criador/postador pode deletar comentário" ON public.community_comments;
CREATE POLICY "Apenas criador/postador pode deletar comentário" ON public.community_comments FOR DELETE USING (auth.uid() = user_id);`;
